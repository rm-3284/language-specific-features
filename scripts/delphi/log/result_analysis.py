...
def get_metrics_per_latent(latent_df: pd.DataFrame) -> pd.DataFrame:
    processed_rows = []
    for score_type, group_df in latent_df.groupby(
        ["module", "latent_idx", "score_type"]
    ):
        conf = compute_confusion(group_df)
        class_m = compute_classification_metrics(conf)
        auc = compute_auc(group_df)
    ...
    df_metrics = pd.DataFrame(processed_rows)
    df_metrics[["layer", "latent_idx", "score_type"]] = pd.DataFrame(df_metrics['score_type'].to_list())
